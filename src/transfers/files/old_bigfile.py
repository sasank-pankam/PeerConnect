...
from src.transfers import thread_pool_for_disk_io, TransferState
...
class Sender(CommonAExitMixIn, CommonExceptionHandlersMixIn, AbstractSender):
    ...
    async def send(self, io_pair, index):
        ...
        for big_chunk in self.file_iterator:
            ...
            # TODO: rewrite this with wire.recv_msg
...
class Receiver(CommonAExitMixIn, CommonExceptionHandlersMixIn, AbstractReceiver):
    ...
    async def recv(self, io_pair: tuple[Callable, Callable], index: int):
        await self._ensure_ok(index)
        ...
        while self.is_stop:
            ...
            big_chunk_id = data.body["file_id"]
            self.current_receiving.add(big_chunk_id)
            new_file = FileItem(
                f"{self.file.path.stem}.{self.id}.{big_chunk_id}.{const.FILE_ERROR_EXT}",
                seeked=0,
            )
            ...
            try:
                ...
                if new_file.seeked < CHUNK_SIZE:
                    ...
                    return
            self.parts[big_chunk_id] = new_file
            self.current_receiving.remove(big_chunk_id)

    async def continue_transfer(self):
        # TODO: maintain the structure same as send_files
        async for _ in self.status_iter:
            yield _
    ...
    async def cancel(self):
        ...
        # TODO: delete all parts

    async def __aexit__(self, exce_type, exec_val, exec_tb):
        # todo check for raised errors
        if exce_type == TransferIncomplete:
            for tk in self.task_group:
